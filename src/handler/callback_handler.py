...
async def proverka_pod(message: Message, state: FSMContext):
    await message.answer('Адрес записали, теперь укажите ваше ФИО\n\n\
Пример: Буханкин Дмитрий Сергеевич')
    await state.update_data(address = message.text)
    await state.set_state(Anketa_arend.name_user)

@router_callback.message(Anketa_arend.name_user)
async def proverka_pod(message: Message, state: FSMContext):
    await message.answer('Ваше ФИО записанно, теперь напишите описание к обьявлению\n\n\
...
async def proverka_pod(message: Message, state: FSMContext):
    await message.answer('Ваше ФИО записанно, теперь напишите описание к вашему обьявлению\n\n\
